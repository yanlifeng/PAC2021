...
float CTF::computeCTF2D(float x, float y, int Nx, int Ny, bool phaseflip, bool flip_contrast,
                        float z_offset) // z_offset in pixels, x和y给的都是格点坐标值，非真实值
{
    float x_norm = (x >= int(ceil(float(Nx + 1) / 2))) ? (x - Nx) : (x), y_norm = (y >= int(ceil(float(Ny + 1) / 2)))
                                                                                  ? (y - Ny) : (y);
    ...
    float df_now =
            ((defocus1 + defocus2 - 2 * z_offset * pix) + (defocus1 - defocus2) * cos(2 * (alpha - astig))) / 2.0;
