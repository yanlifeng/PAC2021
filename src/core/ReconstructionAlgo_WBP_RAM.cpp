        ...
        for(int n=0;n<stack_orig.getNz();n++)   // loop for every micrograph
        {
            ...
            else    // perform CTF correction
            {
                ...
                else    // perform 3D-CTF correction
                {
                    ...
                    {             
                        memcpy(image_now_temp,image_now,sizeof(float)*stack_orig.getNx()*stack_orig.getNy()); // get the raw image!!!
       
                        // correction
                        int n_z=(zz+int(h_tilt_max/2))/defocus_step;
                        
                        ctf_correction(image_now_temp,stack_orig.getNx(),stack_orig.getNy(),ctf_para[n],flip_contrast,float(zz)+float(defocus_step-1)/2);
                        // save
                        stack_corrected[n_z]=new float[stack_orig.getNx()*stack_orig.getNy()];
                        memcpy(stack_corrected[n_z],image_now_temp,sizeof(float)*stack_orig.getNx()*stack_orig.getNy());
                        n_zz++;
                    }
